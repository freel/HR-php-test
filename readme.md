## Сделано
Основное:
В .env добавлена настройка WEATHER_API_KEY, которая хранит API ключ яндекс погоды.
/weather выводит погоду.
После регистрации и входа, в меню доступен пункт "Заказы",отображается список всех заказов с отображаением по 40 элементов на страницу.
По ссылке в ID заказа открывается редактирование заказа.
